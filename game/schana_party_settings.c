...
static SchanaModPartySettings Get()
{
    ...
    // 🔽 deleting admins from player party
    array<string> idsToRemove = GetSchanaPartyServerSettings().GetAdminIds();

    foreach (string id : idsToRemove)
    {
        int index = settings.players.Find(id);
        if (index != -1)
        {
            settings.players.RemoveOrdered(index);
            settings.names.RemoveOrdered(index);
        }
    }
